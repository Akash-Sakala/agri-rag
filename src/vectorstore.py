...
class FaissVectorStore:
    ...
    def add_embeddings(self, embeddings: np.ndarray, metadatas: List[Any] = None):
        ...
        if self.index is None:
            print(f"[INFO] Creating new FAISS index with dimension {dim}")
    ...
    def load(self):
        ...
        print(f"[INFO] Loaded FAISS index + metadata from {self.persist_dir}")

    def search(self, query_embedding: np.ndarray, top_k: int = 5):
        if self.index is None:
            print("[WARN] Attempted search on empty index.")
            return []
